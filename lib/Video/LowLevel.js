...
class Video {
    ...
    async getThumbInfo(videoId) {
        return xml_js_1.xml2js(await this.requestPromise(APIEntryPoints_1.Video.createGetThumbInfoRequest(videoId)));
    }
    /**
    ...
    async getFLV(videoId) {
        return await this.requestPromise(APIEntryPoints_1.Video.createGetFlvRequest(videoId));
    }
    /**
    ...
    async createDmcSession(videoId, apiUrl, session) {
        return JSON.parse(await this.requestPromise(APIEntryPoints_1.Video.createDmcSessionRequest(videoId, apiUrl, JSON.stringify({ session: session }))));
    }
    /**
     *
     * @param {string} videoId
    ...
    async sendDmcHeartbeat(videoId, apiUrl, session) {
        return await this.requestPromise(APIEntryPoints_1.Video.createDmcHeartbeatRequest(videoId, apiUrl, session.id, JSON.stringify({ session: session })));
    }
    async getThreadKey(threadId) {
        return await this.requestPromise(APIEntryPoints_1.Video.createGetThreadKeyRequest(threadId));
    }
