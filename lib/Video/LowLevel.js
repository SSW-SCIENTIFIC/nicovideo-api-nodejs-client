...
class Video {
    ...
    async getThreadKey(threadId) {
        const result = QueryString.parse(await this.requestPromise(APIEntryPoints_1.Video.createGetThreadKeyRequest(threadId)));
    ...
    async getWaybackKey(threadId) {
        return QueryString.parse(await this.requestPromise(APIEntryPoints_1.Video.createGetWaybackKeyRequest(threadId))).waybackkey;
    }
}
exports.Video = Video;
